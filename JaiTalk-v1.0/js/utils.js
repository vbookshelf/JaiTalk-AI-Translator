...
// Function that converts text to speech
//function speak(text, speech_lang_code, speech_voice_name, speech_rate = 1) {
function speak1(text, classList) {
	
	// Get a list of all classes in the element.
	// When the element was created we added the language code as a class name.
	const classes = Array.from(classList);
	
	console.log(classes)

	if (classes.includes(language1_code)) {
		
		speech_voice_name = language1_voice_orig;
		input_lang_code = language1_code;
		
	} else {
		
		speech_voice_name = language2_voice_orig;
		input_lang_code = language2_code;
	...
	/////////////
	 ...
	 // Ensure the language is set
    utterance.lang = input_lang_code;
...
function speakText(innerHtml, classList) {
	...
	speak1(processedText, classList);
}
